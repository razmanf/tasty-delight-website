...
<div x-data="{ 
    ...
    @else
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <!-- Bulk Actions Bar -->
                <div class="flex flex-wrap items-center justify-between p-4 rounded-xl border bg-gray-50 dark:bg-gray-800/50 gap-4" style="border-color: var(--td-border);">
                    <div class="flex items-center gap-3" x-show="isEditMode" style="display: none;">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" wire:model.live="selectAll" class="w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary">
                            <span class="ml-2 text-sm font-bold select-none" style="color: var(--td-text);">Select All</span>
                        </label>
                    </div>
                    <div x-show="!isEditMode">
                        <button type="button" @click="showClearModal = true" class="text-sm py-1.5 px-3 bg-red-100 hover:bg-red-200 text-red-600 rounded-lg transition-colors font-bold shadow-sm">
                            <i class="fa-solid fa-trash-arrow-up"></i> Clear Cart
                        </button>
                    </div>
                    
                    <div class="flex items-center gap-2 flex-wrap">
                        <button type="button" @click="isEditMode = false; $wire.cancelEdit();" x-show="isEditMode" style="display: none;" class="text-sm py-1.5 px-3 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition-colors font-bold shadow-sm">
                            <i class="fa-solid fa-xmark"></i> Cancel
                        </button>
                        <button type="button" @click="showDeleteModal = true" x-show="isEditMode" style="display: none;" class="text-sm py-1.5 px-3 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors font-bold shadow-sm" :disabled="$wire.selectedItems.length === 0" :class="$wire.selectedItems.length === 0 ? 'opacity-50 cursor-not-allowed' : ''">
                            <i class="fa-solid fa-trash-can"></i> Delete
                        </button>
                        <button type="button" @click="isEditMode = true" class="text-sm py-1.5 px-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors font-bold shadow-sm" x-show="!isEditMode">
                            <i class="fa-solid fa-pen"></i> Edit
                        </button>
                        <button type="button" @click="showConfirmModal = true" class="text-sm py-1.5 px-3 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors font-bold shadow-sm" x-show="isEditMode" style="display: none;">
                ...
                <div class="td-card flex flex-col sm:flex-row items-start sm:items-center gap-4 transition-all hover:border-orange-200 group relative"
                    ...
                    <!-- Standard Checkbox -->
                    <div class="flex items-center justify-center flex-shrink-0" x-show="isEditMode" style="display: none;">
                        <label class="cursor-pointer flex items-center">
                            <input type="checkbox" wire:model.live="selectedItems" value="{{ $item->id }}" class="w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary">
                        </label>
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                            ...
                            @php $q = $quantities[$item->id] ?? $item->quantity; @endphp
                            @if($item->product->stock <= 0)
                                <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400">Out of Stock</span>
                            @elseif($item->product->stock < $q)
                                <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400">Only {{ $item->product->stock }} left</span>
                            @endif
                        </div>
            ...
            <div class="td-card h-fit sticky top-24 border-2 border-orange-500/10">
                ...
                <!-- Proceed to Checkout -->
                @if($hasStockIssues)
                <div class="mb-3 text-center p-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-sm font-bold flex items-center justify-center gap-2">
                    <i class="fa-solid fa-triangle-exclamation"></i>
                    Please resolve stock issues before checking out.
                </div>
                <button disabled class="w-full justify-center py-3.5 text-base shadow-sm rounded-xl font-bold flex items-center bg-gray-200 dark:bg-gray-800 text-gray-500 cursor-not-allowed">
    ...
    <div x-show="showConfirmModal"
        ...
        <div class="td-card relative z-10 max-w-sm w-full p-6 text-center transform shadow-2xl"
            ...
            <div class="w-16 h-16 rounded-full mx-auto flex items-center justify-center mb-4 shadow-inner" style="background: var(--td-primary); color: white;">
            ...
            <p class="text-sm mb-6" style="color: var(--td-muted);">
                Are you sure you want to apply these quantity changes?
            </p>
    ...
    <div x-show="showDeleteModal"
        ...
        <div class="td-card relative z-10 max-w-sm w-full p-6 text-center transform shadow-2xl"
            ...
            <div class="w-16 h-16 rounded-full mx-auto flex items-center justify-center mb-4 shadow-inner bg-red-500 text-white">
                <i class="fa-solid fa-trash-can text-2xl"></i>
            </div>
            
            <h3 class="text-xl font-bold mb-2" style="color: var(--td-text);">Delete Items?</h3>
            <p class="text-sm mb-6" style="color: var(--td-muted);">
                Are you sure you want to delete <span x-text="$wire.selectedItems.length"></span> selected items from your cart?
            </p>
            
            <div class="flex gap-3 w-full">
                <button @click="showDeleteModal = false" class="flex-1 py-2.5 rounded-xl font-bold bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 transition-colors" style="color: var(--td-text);">
                    Cancel
                </button>
                <button @click="$wire.deleteSelected(); showDeleteModal = false; isEditMode = false;" class="flex-1 py-2.5 rounded-xl font-bold text-white shadow-md transition-transform hover:scale-105 bg-red-500 hover:bg-red-600">
                    Delete
                </button>
            </div>
        </div>
    ...
    <div x-show="showClearModal"
        ...
        <div class="td-card relative z-10 max-w-sm w-full p-6 text-center transform shadow-2xl"
            ...
            <div class="w-16 h-16 rounded-full mx-auto flex items-center justify-center mb-4 shadow-inner bg-red-600 text-white">
                <i class="fa-solid fa-triangle-exclamation text-2xl"></i>
            </div>
            
            <h3 class="text-xl font-bold mb-2" style="color: var(--td-text);">Clear Entire Cart?</h3>
            <p class="text-sm mb-6" style="color: var(--td-muted);">
                Are you sure you want to completely empty your cart? This action cannot be undone.
            </p>
            
            <div class="flex gap-3 w-full">
                <button @click="showClearModal = false" class="flex-1 py-2.5 rounded-xl font-bold bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 transition-colors" style="color: var(--td-text);">
                    Cancel
                </button>
                <button @click="$wire.clearCart(); showClearModal = false;" class="flex-1 py-2.5 rounded-xl font-bold text-white shadow-md transition-transform hover:scale-105 bg-red-600 hover:bg-red-700">
                    Clear Cart
                </button>
            </div>
        </div>
